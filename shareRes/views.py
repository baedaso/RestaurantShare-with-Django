...
def index(request):
    ...
    return render(request, 'shareRes/index.html', content) #shareRes 파일 밑에 index.html이라는 뜻
...
def Update_restaurant(request):
    ...
    return HttpResponseRedirect(reverse('reDetailPage', kwargs={'res_id':resId}))
...
def Create_category(request):
    ...
    new_category.save()
    return HttpResponseRedirect(reverse('index'))
    #return HttpResponse("여기서 카테고리 크리에이트 기능을 구현할 거야")

def Delete_category(requet):
    category_id = requet.POST['categoryId']
